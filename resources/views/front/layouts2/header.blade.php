              ...
              <div class="sidebar" id="sidebarMenu">
                ...
                <div class="signin-register" style="text-align: center; padding: 0 0 16px 0;">
                  <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;">
                    ...
                    <a href="{{route('customers.profile')}}"> <img src="/front/images/Profile_img.svg" alt="Account" style="width: 32px; height: 32px; margin-left:20px;margin-top:20px" /></a>
                    @else
                    <a href="{{route('login')}}"> <img src="/front/images/Profile_img.svg" alt="Account" style="width: 32px; height: 32px; margin-left:20px;margin-top:20px" /></a>
                    @endif

                    @if (Auth::check() && Auth::user()->hasRole('Customer'))
                      {{-- {{ ucfirst(strtolower(explode(' ', Auth::user()->name)[0])) }} --}}
                      <a href="{{route('customers.profile')}}">  <span style="font-size: 20px; font-weight: 600; color: #111; margin-top:20px">{{ ucfirst(strtolower(explode(' ', Auth::user()->name)[0])) }}</span></a>
                    @else
              ...
              <div class="new-search-bar">
                ...
                <button type="submit" class="new-search-btn" onclick="searchproductinput(document.getElementById('searchInput').value);">
                  <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
                    ...
                    <line x1="15.4142" y1="15" x2="20" y2="19.5858" stroke="#fff" stroke-width="2" stroke-linecap="round"/>
                  </svg>
                </button>
              </div>
              @endif
